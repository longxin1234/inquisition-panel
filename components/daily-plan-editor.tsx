            ...
            {items.map((node, index) =>
              node.type === "fight" ? (
                <SortableShell key={`fight-${index}`} id={rootId(index)} className="items-center p-2" contentClassName="flex items-center gap-2">
                  ...
                  <Input type="number" min="1" max="99" value={node.fight.num} onChange={(e) => updateNode(index, { ...node, fight: { ...node.fight, num: Math.min(99, Math.max(1, Number.parseInt(e.target.value) || 1)) } })} className="w-24 dark:bg-gray-600 dark:border-gray-500 dark:text-white" />
                      ...
                      <div className="space-y-2">
                        {node.loopGroup.items.map((item, groupIndex) => (
                          <SortableShell key={`group-item-${groupIndex}`} id={loopId(index, groupIndex)} className="items-center border-dashed bg-white/60 p-2 dark:bg-gray-800/50" contentClassName="flex items-center gap-2">
                            ...
                            <Input type="number" min="1" max="99" value={item.weight} onChange={(e) => updateNode(index, { ...node, loopGroup: { ...node.loopGroup, items: node.loopGroup.items.map((current, currentIndex) => currentIndex === groupIndex ? { ...current, weight: Math.min(99, Math.max(1, Number.parseInt(e.target.value) || 1)) } : current) } })} className="w-24 dark:bg-gray-600 dark:border-gray-500 dark:text-white" />
